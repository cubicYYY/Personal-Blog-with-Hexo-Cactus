...
以**快速复习**为目的，尽可能对缺乏动机(intuition)的一些概念进行简明解释。
标题中使用`*`标记的部分为考试**不**考察的内容。

文中大部分图片与文字来自于姚老师(ZJU, pyaoaa at zju.edu.cn)
...
- 半开卷，允许携带 3 张双面**打印或手写**的 A4 纸入场
- 考试范围：虎书 1-11 13-14 18 章
- 题型：判断30道\*1分 选择20道\*1分 大题共50分
- **复习重点：重要概念、算法 提高计算题熟练度 确保掌握作业题**
...
将输入字符串识别为有意义的子串。（把基本单元划分好）
(Token单词, Lexeme词素(可选)) e.g. (IF, ) (ELSE, ) (BINARY_OP, >=) (UNARY_OP, &)
...
例如有产生式 $A\rightarrow \gamma$，我们可以有 $\alpha A \beta \Rightarrow \alpha \gamma \beta$
那么我们说：
...
3. 左公因子的(left-factored)文法:
    ...
    - 解决办法：限制文法 或 文法变换
      - 例如我们可以提取左公因子来“推迟决定”，这样我们可以在读入更多token后进行决策：
         把
         $$ P \rightarrow \alpha \beta | \alpha \gamma $$
         变换为
...
从串w归约为文法开始符号S的过程。规约时，一个与某产生式体**相匹配的特定子串**被替换为该产生式头部的**非终结符号**。
...
例如产生式 $A\rightarrow XYZ$ 有4个Item:
...
- 属性:= 描述文法符号的语义特征，比如表达式E的值我们可以记为E.val
...
例如我们通过如下语义动作可以在编译期直接evaluate表达式的值：
...
因此假设T和F两个token的关联类型都是`int`，对于表达式 $T\rightarrow T * F$ 我们可以如此实现语义动作：
...
- 产生式统一放进一个union（如果是Rust就是Option直接解决，~~C语言太坏了~~）里，每一个产生式就是union里的一个结构体，这个结构体用于储存其子节点 ![](AST-def.png)
- 为每个产生式定义一个函数，除了计算需要的语义值返回以外，还将申请空间、分配新的树节点并设置好其子节点 ![](AST-impl.png)
...
- parser维护*位置栈*与*语义值栈*，这样语义操作就知道位置信息了
  ...
  - 对于Yacc等无法直接实现的，可以引入新的非终结符pos（其语义值包含需要的位置信息）并改写文法。例如我们可以如此改写PLUS表达式以利用位置信息：
...
我们注意到上述的属性文法等内容只适用于上下文无关文法CFG.
然而CFG有很多不足，例如我们不可能完成：
...
这是因为，这些检查和值有关（涉及语义），而非语法本身。因此我们需要通过检查、遍历**程序表示(Program Representation)**来完成**（广义的）语义分析**。
常用的程序表示：
...
在本课中，我们重点关注的（狭义的）语义分析指的是通过检查AST获知程序的静态属性，包括：
...
- 符号表的实现：
  绑定时，如果遇到了符号已经存在的情况，我们有两种策略：
  - Imperative Style: 直接覆盖旧的绑定，这样我们不可能lookup到旧的信息。当这个新的绑定不再有效时，需要复原旧的绑定。
  ...
  - Functional Style: 永远保留老的，只是查询时做一些额外处理（可以理解为只是renaming）。这样还原更简单。
    - 直接使用BST（红黑树等）实现查找
    - 可以使用可持久化数据结构完成删除、复原等操作，进一步降低单次操作的空间复杂度，非常方便
...
Tiger编译器的environment是destructive-update的。也就是说，我们有两个函数：
...
- endScope: 一直弹出符号直到弹出了一个特殊标记
  - 我们可以由此标记推断：此次因为退出scope引发的restore操作可以就此结束
...
1. 类型系统
    ...
    > 事实上，我们可以理解为每引入一种类型就能**完全**消除某一类特定错误。
...
- Caller-save: 例如临时寄存器，调用者如果用到则需要自己保存，子函数可以任意修改
- Callee-save: 例如FP/SP, 由子函数负责保存与恢复（进入子函数时push到栈，退出时从栈里pop），调用者无需关心
...
以及有时变量“逃逸(escape)”了（也就是脱离了当前scope/无法确定变量有效的生命周期）：
...
![](nested-funcs.png)
我们可以通过FP访问（因为定义的变量内存地址在编译时未知，**但相对当前函数的FP偏移值(offset)是已知的**）。
...
> 💡本质上，我们在做的事和Lab3里为每一个参数分配地址，把一切读写放在内存里是一样的。只是为了效率，有时这个“地址”并不存在而是储存在寄存器中；此外有的变量会逃逸。因此我们需要实现访问的接口，对在寄存器的变量或者会逃逸的变量进行一些额外处理。
...
例如，`ESEQ(a=5, a+5)`会返回值`10`, 同时副作用是`a`的值变为5
...
2. 变量翻译
    - 普通变量
      - 相对FP的便宜是固定的。假设偏移量为`k`，则内存中变量的值为：`MEM(BINOP(PLUS,TEMP fp, CONST k))`
      - 注意，对于一些常见的操作（例如加法）我们有如下简写：`BINOP(PLUS, a, b) = +(a, b)`
...
- 类型、变量与函数定义
  ...
  - 函数定义分为三部分：
    - prologue:
      ...
      - 保存Callee-save的寄存器到栈帧（例如返回地址）
    - body: 翻译的函数体
    - epiloge:
      - 保存返回值到寄存器（或栈上某地址）
      - 从栈帧中恢复Callee-save的寄存器
      - 回退到调用者的栈帧（更改FP/SP）
      - Return指令
      - 【可选】伪指令标记函数在此结束
...
在将IR的AST翻译为机器码的过程中，我们不可避免地需要解决将“树形的”AST转换到“线性的”机器码这一问题。
...
1. IR树被转化/重写，或者说线性化为一些正规树。
   基本上干的事情就是消除`ESEQ`以及把`CALL`向上挪。最后把树中的`SEQ`删去，这棵树就会“破碎”成很多**正规树**。
...
一种中间体。从IR Tree线性化而来，又到正规形式之中去。
不要太纠结这个概念。*正规树*，不过是因为要重排一些指令，不得不破碎原来的树为森林后，为这些树起的一个过于术语化的名字。

性质：

- 其他所有节点都是**表达式**。
- 根节点为 **语句(statement)** ，所以正规树就对应IR中的一个语句，也就是IR Tree中的一个子树。
  - 只不过有额外约束，所以经过了变换结构可能不一样（也可能由于原来的子树已经很纯很“正规”所以没变化）。
- 不存在`SEQ`或者`ESEQ`。
...
这些树最后会被组装为canonical form. 具体步骤见下。
...
1. 消除`ESEQ`
    首先我们要去除原先树中所有的ESEQ. 总的来说是不断向上“提升”`ESEQ`直到它可以转为`SEQ`.
    部分规则如下：
    ...
    这些变换的成立性是容易验证的，
    在这之中，我们要注意副作用的问题：也就是语句执行顺序对结果有影响的情况。例如对于`BINOP(op, e1, ESEQ(s, e2))`（`e1`先求值，而后执行`s`）：

    - 如果`s`会影响`e1`的值，那么我们不能调换二者的执行顺序。
    - 也就是说**不一定能**变形为`ESEQ(s, BINOP(op, e1, e2))`（`s`先执行，而后对`e1`求值）
    - 解决方法：使用临时变量保存`e1`值。这种情况下我们变形为`ESEQ(MOVE(TEMP t,e1), ESEQ(s, BINOP(op, TEMP t, e2)))`

    可见，取决于`s`是否会影响`e1`值，我们有不同的变形策略。
    事实上，“会不会影响取值”是一种我们称为“可交换性(commutativity)”的属性：

    - 如果`s`**不**影响`e1`值，那么我们说`s`, `e1` commute
    - 如果`s`**会**影响`e1`值，那么我们说`s`, `e1` do NOT commute

    如果能判断两者是否commute, 那么我们就可以最大限度避免引入新的临时变量，使得目标代码尽可能短小、高效。遗憾的是由于内存读写等原因，这一判定问题是困难的。不过我们只需要进行保守近似即可。也就是说，如果我们的保守近似策略判定二者commute, 那么他们一定commute; 否则如果策略无法确信二者commute, 我们则保守地认为他们互相有可能影响（不commute）。
...
3. 重排并消除`SEQ`
    ...
    我们现在需要把`SEQ`变为父节点的最右儿子（类似平衡树的旋转）。这只需要无脑应用`SEQ(SEQ(a, b), c)` => `SEQ(a, seq(b, c))`即可。
...
因此我们需要重排树，让`CJUMP`后紧接的就是一个label, 对应其中一个分支跳转到的块。
这需要以下两步：
...
1. Stage 2: 组装为基本块(basic block)
    ...
    - 中间不包含其他label/terminator

    本质上，如果没有跳转指令，程序只会一路向下执行。正是跳转指令（JUMP/CJUMP/RET/...）令程序在执行中途跳出，而跳转的目标地址（也就是LABEL）令程序在执行中途跳入，才把原本的代码分割为了多个基本块。
...
2. Stage 3: 生成（许多）trace
    ...
    通过这种方式，我们可以去掉不少跳转。
    这是因为现在可以转变思维：原先我们有似乎毫无关联的跳转指令，而现在则是把其中一些连在一起成为traces，然后默认代码执行是在一个trace上走到底的。于是只有要切换trace时才需要一个跳转。
...
3. **指令选择**：从canonical trees产生伪汇编代码
...
回顾[现代编译器架构](#intermediate-representation)：**指令选择** 开始，我们进入编译器的后端部分。
...
最好= 最快/最小/最省电……这个判据取决于具体需求。总之我们需要综合考虑操作数/结果的访存需求，指令本身的代价等。
...
为了方便起见，我们接下来假设目标机器码是一个简单的架构（指令集）：`Jouette`架构。它基本上就是RISC-V的子集。我们约定：
...
- 通过寻找树上的模式(tree patterns)
  ...
  - 目标：没有重叠地覆盖整棵IR Tree. 我们把这个过程称为`tiling`
...
现在我们要做的是找到一种方法，使得生成的机器代码跑的比香港记者还快。
...
我们可以用很多小的tile轻松覆盖整棵树，但是大量tile对应大量目标代码，很长很慢；
...
- 动态规划(dynamic programming): optimum tiling (全局最优的)
  - 自底向上
...
> 这里需要特别注意：tile 的子树不是该节点的子树，而是用 tile 覆盖该节点以及部分子节点后，那些没被覆盖到的子树。
...
**简单粗暴的理解：活跃变量=还有用的变量，需要留着备用；不活跃变量=没有用的变量，不再使用，对应的寄存器可以释放。**

为什么要进行活跃性分析呢？因为从Low level IR到机器码的翻译过程中，我们需要把有限但很多的抽象寄存器对应到物理寄存器上，也就是**寄存器分配**。
如果能识别不需要使用的变量（i.e. 不活跃变量），那么其对应的物理寄存器就可以被释放，再次用于分配。

> 我们可以把活跃变量分析理解为一种特殊的生命周期分析，只不过除了显式的变量，我们还要考虑那些中间值。

活跃性分析用途：

- 指导/优化寄存器分配，减少物理寄存器使用
  例如对于：

  ```plaintext
  1: a = 1; // a lives in 1-2
  2: b = a + 2; // b lives in 2-3
  3: c = b + 3; // c lives in 3-4
  4: return c;
  ```
...
首先要明确一点：我们不可能精确知道一个变量在某处是否活跃，所以我们要做的只是**保守估计**（i.e. 标记为不活跃的变量一定不活跃，但反之则不然）。
这是因为对于代码片段`int x=10; f(); return x;`:
如果我们能判断x是否在定义并初始化**之后**依旧活跃，就等于我们能判断`f()`是否停机。而众所周知停机问题不可判定，so sad.
...
- fact: 从CFG中能推断出的一些结论/信息/事实，这些结论是“绑定”在有向边上的
  - 例如我们现在研究的变量活跃性(liveness)就是一个fact. 变量是否活跃这一信息是通过这些有向边传递的。
- in\[n\]: 一个集合，包含节点`n`的入边(in-edges)上的所有facts
  - 对于本节来说，我们要求的是变量的liveness, 所以为在节点`n`内 live-in(定义见下文) 的变量。
    ...
    - **我们可以简单理解为：在节点`n`中的语句执行后，哪些变量处于活跃状态，有可能在未来被用到**
  - 我们稍后会详细解释该集合的含义，以及如何计算它。
- out\[n\]: 一个集合，包含节点`n`的出边(out-edges)上的所有facts
  - 对于本节来说，我们要求的是变量的liveness, 所以为在节点`n`内 live-out(定义见下文) 的变量。
    ...
    - **我们可以简单理解为：在节点`n`中的代码执行前，哪些变量处于活跃状态，有可能在当前节点`n`或未来被用到**
  - 我们稍后会详细解释该集合的含义，以及如何计算它。
- transfer function: 一个函数，定义一个节点上的信息如何通过传递给另一个节点
  - 比如我们稍后会得到的两个in/out的方程（等式）
- use: 如果一个语句的右侧(RHS)出现了变量`x`, 我们说这个语句use了`x`
  - 这意味着变量`x`中储存的值被使用了，引入了一个数据依赖。
- use of a variable `x`: 所有use了`x`的节点集合
- define: 对`x`赋值即为define了`x`
...
- `x` is **live on an edge:** 存在一条以这条有向边(edge)开始的路径，终点指向一个use of `x`, 且中间经过的节点不包含任何def of `x`
  - 这意味着：`x`可能在之后被使用；而且在被使用前没有被重定义/赋值过（也就是要保证数据确实可能存在依赖：那个 use 必须真的使用现在这个`x`对应的值）。
...
记住：CFG 中的一个结点是一段代码，但是为了方便起见我们都假设只有一个语句在里面。所以其实你可以很简单地合并一串连续的代码（变成基本块）。**我们说“执行某个节点”，指的就是执行节点里的代码。**

我们接下来的重点就是找到一种方式计算in/out集合。
事实上我们可以迭代地不断计算、更新`in[n]`以及`out[n]`, 直到他们在某次迭代后不再改变变（i.e. 到达了这个算法的*不动点*）。
...
3. If $ a \in out[n] $ and $ a \notin def[n] $ , then $ a \in in[n] $
   - 解释：如果现在已知某个变量`a`必须在节点`n`执行完毕时活跃，但是`n`自己并没有定义`a`, 那显然`a`是从之前的前驱节点继承而来。
     因此，我们要求`a`在节点`n`执行之前就是活跃的。

从这些规则，我们可以导出如下两个方程（等式），**可直接用于迭代计算并更新 in\[n\] 与 out\[n\]** 这两个**集合**:
...
应用这两个方程，我们可以给出如下算法求解出in/out：

```plaintext
for each n
  in[n] ←{}; out[n] ←{} // init. to empty set, which is a very rough approximation!
repeat:
  for each n
    in′[n] ← in[n]; out′[n] ← out[n]
    
    // `U` stands for "union"
    in[n] ← use[n] U (out[n] − def[n]) // use/def is easy to obtain at the very beginning
    out[n] ← ⋃ in[s], where s in succ[n] 

until in′[n] = in[n] and out′[n] = out[n] for all n // until no update can be made
```
...
> liveness 就是一种 fact.
> 我们的迭代更新顺序也应遵循这些 facts 的流动方向。而在活跃性分析中，liveness 反向流过这些 CFG 的有向边，并且是先影响 out 再进而影响 in.
> 因此我们迭代的过程也要遵从这一规律，使得一次迭代中，某一个 use 引入的需求能够迅速传遍整个 CFG, 而非每次只影响附近的个别结点，
>
...
- 变量活跃性可以分为：
  ...
  - 静态的static liveness: 像我们刚刚做的一样在编译期判断，显然是一种over-approximation, 因为估计得很保守，而事实上有些路径不可能被执行到
