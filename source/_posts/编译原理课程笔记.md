...
- 问题：即使如此，栈的大小太大了
  - 解决方法：**Deutsch-Schorr-Waite (DSW) pointer reversal**
...
- 编译器的任务：
  ...
  - 为 GC 提示哪些根变量包含指针，从而作为每次 GC 的起点：
    ...
    - Tiger 语言的解决方法：使用 **pointer map**, 编译期就能知道哪些 temporary 是指针。
      ...
      - 需要特殊处理 callee-save 寄存器：考虑调用链 f->g->h, g 在调用 h 前，对于这些 callee-save 寄存器，必须在 pointer map 中额外记录哪些是 g 自己使用的，哪些是从 f "继承"来的。
- Derived Pointer:
...
- 如何判断`x`是不是类`C`的实例？
  ...
  - 更快的做法：**display**.
    ...
    - 由于对每个类，其继承关系的嵌套深度在编译期已知，因此可以立刻找到需要比较 display 中的哪一项。
      - 例如，假设`MeClass`的继承深度是第 3 层，要检查`x`是否为`MeClass`的实例，只需检查`x`的继承深度是否大于等于 5, 且`x.0.display[5]`是否指向`MeClass`的 class descriptor 即可。
