...
换句话说就是 BFS 的过程中，每次复制对象`x`时，顺带把一条路径（`x`->某个叶子）上的对象也复制了。
...
- Natural loop: 自然循环
  ...
  - 我们可以基于 back edge 严谨地定义 CFG 图中的一个自然循环：
    ...
    **说人话就是 back edge 是回到循环头(loop header)的边。** 至少有一条路径能返回首节点`h`: 循环尾部回到循环头部的那条边；循环体就是中间那些被 loop header 支配的节点。
  - 循环可以嵌套：
...
- Loop-nest tree: 表达循环的嵌套关系
  ...
  ![loop-nest-tree](loop-nest-tree.png)
